...
| 基础信息 | `basic.csv` | 过滤股票名称、退市名称和北交所股票 | 由使用者本地准备 |
| ST 列表 | 每日 `stock_st` 文件 | 按信号日过滤 ST 股票 | 由使用者本地准备 |
...
1. 在侧边栏确认 `项目根目录`、`checkpoint`、`scaler.json`、`basic.csv`、每日 `stock_st`、`TopK=10`、`DropK=2`、`signal_date` 和计划交易日期。
...
| 缺少 `--basic_csv` 或 `--stock_st` | 预测现在强制要求同时提供基础信息和每日 ST 文件 |
| 候选中出现不合规股票 | 再次核对 `basic.csv`、每日 `stock_st` 和北交所过滤结果 |
...
python src/predict_daily.py `
  ...
  --basic_csv <basic_csv_path> `
  --stock_st <daily_stock_st_csv_path> `
...
2. 每日股票池必须排除北交所与 ST 股票；`predict_daily.py` 现在强制要求 `--basic_csv` 和 `--stock_st`，分别用于过滤名称含 `ST/*ST/退市`、北交所股票和每日 ST 名单。
...
2. GitHub 中存在 requirements.txt，且包含 streamlit、pyarrow、scipy。
