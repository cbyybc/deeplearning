...
├── configs/
│   ├── config.json                  # MLP 配置
│   ├── config_lstm.json             # LSTM seq20 配置
│   └── config_lstm_seq10.json       # LSTM seq10 配置，可选
│
├── src/
...
│
├── outputs/                         # MLP 输出
├── outputs_lstm/                    # LSTM seq20 输出
├── outputs_lstm_seq10/              # LSTM seq10 输出，可选
│
...
```python
feature_cols = [
    "close_rank", "vol_rank", "amount_rank",
    "amplitude", "body_ratio",
    "ret_1", "ret_5", "ret_10", "ret_20",
    "rel_ma5", "rel_ma10", "rel_ma20",
    "rsi_14",
    "macd_dif", "macd_dea", "macd_bar",
    "vol_std_10", "vol_std_20",
    "vol_chg", "amount_chg",
]
```
...
原因是不同股票价格尺度差异较大，直接输入可能干扰模型。当前版本更偏向使用相对特征和标准化后的技术指标。
...
```text
1. MLP baseline
2. LSTM seq20 baseline
3. LSTM seq10 baseline
```
...
```bash
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python robust_backtest_analysis.py   --config configs/config.json   --model_path outputs/models/best_mlp.pt   --out_dir outputs/robust_backtest_oc_label   --top_k 10   --drop_k 2   --fee_rate 0.0003   --random_runs 20
```
...
Buffer-Risk 收益略低于 Top10 Full，但更稳健、更适合作为主策略。
```
...
ICIR: 0.16411
RankIC_mean: -0.00350
DirectionAcc: 0.50596
valid_loss: 0.00039568
```
...
| 策略 | 总收益 |
|---|---:|
| LSTM seq20 Top10 Full | 111.74% |
| LSTM seq20 Top10-Drop2 | 69.43% |
| LSTM seq20 Buffer-Risk | 55.32% |
...
```text
LSTM seq20 具有一定预测能力，其中 Top10 Full 明显超过随机策略。
但是 seq20 在 Drop2 和 Buffer-Risk 策略下表现一般。
```
...
ICIR: 0.17036
RankIC_mean: -0.00894
DirectionAcc: 0.49888
valid_loss: 0.00040218
```
...
| 策略 | 总收益 |
|---|---:|
| LSTM seq10 Top10-Drop2 | 129.42% |
| LSTM seq10 Buffer-Risk | 85.98% |
| LSTM seq10 Top10 Full | 75.32% |
...
```text
LSTM seq10 明显优于 seq20 的 Drop2 策略，说明较短时间窗口更适合 label_oc_1d 这种短期预测目标。
LSTM seq10 在 Top10-Drop2 策略下表现最好，并超过对应随机策略。
```
...
| 模型 | 窗口 | IC_mean | ICIR | RankIC_mean |
|---|---:|---:|---:|---:|
| MLP | 单日特征 | 0.02265 | 0.18847 | -0.01660 |
| LSTM | 20 | 0.02006 | 0.22324 | -0.01475 |
| LSTM | 10 | 0.01985 | 0.19592 | -0.02923 |

三个模型的 IC 均为正，说明都具有一定预测能力。

RankIC 均为负或接近 0，说明模型对全市场完整排序能力有限，但 TopK 策略只使用高分股票池，因此仍可能取得较好收益。
...
| LSTM seq20 | 111.74% | 69.43% | 55.32% |
| LSTM seq10 | 75.32% | 129.42% | 85.98% |

结论：

```text
1. MLP 在 Top10 Full 和 Buffer-Risk 策略下表现最好。
2. LSTM seq10 在 Top10-Drop2 策略下表现最好。
3. LSTM seq20 对全换仓策略更友好，但对 Buffer-Risk 不适配。
...
虽然 MLP Top10 Full 收益最高，但换手率较高，真实交易中更容易受到手续费、滑点和成交约束影响。
...
```bash
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python src/train_lstm.py   --config configs/config_lstm.json
```

### 14.5 LSTM seq20 稳健回测

```bash
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python src/robust_backtest_lstm.py   --config configs/config_lstm.json   --model_path outputs_lstm/models/best_lstm.pt   --out_dir outputs_lstm/robust_backtest_lstm   --top_k 10   --drop_k 2   --fee_rate 0.0003   --random_runs 20
```

### 14.6 训练 LSTM seq10

```bash
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python src/train_lstm.py   --config configs/config_lstm_seq10.json
```

### 14.7 LSTM seq10 稳健回测

```bash
PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python python src/robust_backtest_lstm.py   --config configs/config_lstm_seq10.json   --model_path outputs_lstm_seq10/models/best_lstm.pt   --out_dir outputs_lstm_seq10/robust_backtest_lstm   --top_k 10   --drop_k 2   --fee_rate 0.0003   --random_runs 20
```
...
5. LSTM loss_curve_lstm.png
6. LSTM monthly_ic_lstm.png
7. LSTM strategy_nav_comparison_lstm.png
8. LSTM strategy_drawdown_comparison_lstm.png
```
...
outputs/models/
outputs_lstm/models/
outputs_lstm_seq10/models/
outputs/predictions/
outputs_lstm/predictions/
outputs_lstm_seq10/predictions/
```
...
3. 完成 MLP baseline
4. 完成 LSTM seq20 baseline
5. 完成 LSTM seq10 baseline
6. 完成随机策略对比
7. 完成真实执行假设下的稳健回测
8. 完成多模型、多策略对比
```
...
```text
1. MLP 是当前整体最稳的 baseline。
2. LSTM seq10 在 Top10-Drop2 策略下表现最好。
3. LSTM seq20 不如 seq10 适合短期 label_oc_1d。
...
```text
本文首先构建了 MLP baseline，用单日量价技术指标预测未来短期收益分数。随后，为了检验序列建模是否能够提升预测效果，进一步实现了 LSTM 模型，并分别测试了 20 日和 10 日历史窗口。实验结果表明，MLP 在整体收益和 Buffer-Risk 策略下表现较稳健，而 LSTM seq10 在 Top10-Drop2 策略下取得最高收益。这说明不同模型产生的预测信号具有不同特性，需要与合适的交易策略结合。
