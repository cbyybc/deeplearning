...
当前预测脚本已经强制要求同时提供 `basic.csv` 和每日 `stock_st` 文件。缺少任意一个文件时，预测会直接停止，不会生成候选股票。
...
1. 通过 `name` 过滤名称含 `ST`、`*ST`、`退市` 的股票。
2. 通过 `market` 过滤北交所股票。
3. 给候选清单补充股票名称，方便人工复核。

建议放在本地固定位置，例如：

```text
data/stock_status/basic.csv
```

### 5.2 下载每日 `stock_st` 文件
...
每日预测时，选择与 `signal_date` 对应的每日 ST 文件，或选择不晚于 `signal_date` 的最新 ST 文件。

例如你用 `2026-05-22` 的盘后行情生成信号，应优先使用：

```text
stock_st/20260522.csv
```
...
预测脚本会读取该文件中 `trade_date <= signal_date` 的记录，并只使用其中最新一个 `trade_date` 的 ST 名单，避免历史复现时误用未来 ST 信息。

建议放在本地固定位置，例如：

```text
data/stock_status/stock_st/20260522.csv
```
...
### 7.3 手动确认 `basic.csv` 和每日 `stock_st`
...
1. `basic.csv` 指向基础数据中的 `basic.csv`，不是日行情文件。
2. 每日 `stock_st` 指向 `stock_st/` 目录下的某一天 ST 文件，通常与 `signal_date` 日期一致。
3. 两个文件都不要放进 `data/local_market_data/` 当行情读取。

### 7.4 手动决定 `signal_date`
...
4. `--basic_csv` 是否指向基础数据的 `basic.csv`。
5. `--stock_st` 是否指向每日 ST 文件。
6. `--seq_len` 是否为 `10`。
7. `--trade_date` 是否为下一交易日。
...
3. Top 50 候选不是明显异常数据。
4. 没有出现 `.BJ` 股票。
5. 没有出现名称含 `ST`、`*ST` 或 `退市` 的股票。
6. 如果预测停止并提示缺少 `basic.csv` 或 `stock_st`，先补齐侧边栏两个必填路径再运行。
...
3. 获取或确认当天对应的 `basic.csv` 和每日 `stock_st` 文件。
4. 确认累计目录中历史数据仍在。
5. 打开 dashboard。
6. 核对当前持仓文件。
7. 填写 `basic.csv（必填）` 和 `每日 stock_st 文件（必填）`。
8. `signal_date` 留空或手动锁定信号日。
9. 手动设置下一交易日 `trade_date`。
10. 运行每日预测。
11. 检查信号日和候选股票。
12. 生成交易计划。
13. 下载或查看买入、卖出、持有清单。
...
### 13.2 `No valid sequence generated`
...
### 13.5 缺少 `basic.csv` 或每日 `stock_st`

说明预测脚本没有拿到强制要求的股票基础信息和每日 ST 名单。

手动检查：

1. 侧边栏 `basic.csv（必填）` 是否已填写。
2. 侧边栏 `每日 stock_st 文件（必填）` 是否已填写。
3. `basic.csv` 是否至少包含 `ts_code/name/market`。
4. `stock_st` 文件是否至少包含 `ts_code/trade_date`。
5. `stock_st` 日期是否不晚于本次 `signal_date`。
...
1. 当天上传的最新行情日期。
2. 当天使用的 `basic.csv` 路径。
3. 当天使用的每日 `stock_st` 文件路径。
4. 当天信号日。
5. 实际交易日。
6. `latest_candidates.csv`。
7. `latest_trade_plan.csv`。
8. 实际成交记录。
9. 更新后的 `current_positions.csv`。
10. 模拟盘收益、持仓和交易截图。
