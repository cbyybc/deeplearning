...
def make_signal_frame(cfg, model_path):
    ...
    keep_cols = ["ts_code", "trade_date", "open", "close", "ret_1", "ret_5", "vol_std_20", "amount_ma20"]
    ...
    model = DLinearRegressor(
        ...
        moving_avg=ckpt.get("moving_avg", cfg["sequence"].get("moving_avg", 3)),
        dropout=cfg["train"].get("dropout", 0.1),
    ).to(device)
    model.load_state_dict(ckpt["model_state_dict"])

    sig = predict_sequence(model, bt_ds, cfg["train"]["batch_size"], device, cfg["train"].get("num_workers", 0))
    ...
    exec_price = exec_price.rename(columns={"trade_date": "exec_date", "open": "exec_open", "close": "exec_close"})
...
def run_strategy(sig, strategy, top_k=10, drop_k=2, fee_rate=0.0003, seed=42):
    ...
    for d in dates:
        ...
        if strategy in ["dlinear_top10_full", "random_top10_full"]:
        ...
        elif strategy in ["dlinear_top10_drop2", "random_top10_drop2"]:
            if len(positions) == 0:
        ...
        elif strategy == "dlinear_buffer_risk":
            ranked = day.copy()
...
def plot_nav(nav_dict, out_dir):
    ...
    plt.xlabel("Date")
    plt.ylabel("NAV")
    plt.title("DLinear Strategy NAV Comparison")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "strategy_nav_comparison_dlinear.png"), dpi=200)
    plt.close()
    ...
    plt.xlabel("Date")
    plt.ylabel("Drawdown")
    plt.title("DLinear Strategy Drawdown Comparison")
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "strategy_drawdown_comparison_dlinear.png"), dpi=200)
    plt.close()
...
def monthly_ic_plot(pred_df, out_dir):
    ...
    monthly.to_csv(os.path.join(out_dir, "monthly_ic_dlinear.csv"), index=False, encoding="utf-8-sig")
    ...
    plt.title("DLinear Monthly IC")
    plt.xticks(rotation=45)
    plt.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(out_dir, "monthly_ic_dlinear.png"), dpi=200)
    plt.close()
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="outputs_dlinear_seq10_e60/models/best_dlinear.pt")
    parser.add_argument("--out_dir", type=str, default="outputs_dlinear_seq10_e60/robust_backtest_dlinear")
    ...
    print("=" * 80)
    print("生成 DLinear realistic signal predictions")
    print("=" * 80)

    sig = make_signal_frame(cfg, args.model_path)
    sig.to_csv(os.path.join(args.out_dir, "realistic_signal_predictions_dlinear.csv"), index=False, encoding="utf-8-sig")
    ...
    save_json(pred_metrics, os.path.join(args.out_dir, "realistic_prediction_metrics_dlinear.json"))
    monthly_ic_plot(pred_df, args.out_dir)

    print("=" * 80)
    print("DLinear Realistic prediction metrics")
    print("=" * 80)
    ...
    for st in ["dlinear_top10_full", "dlinear_top10_drop2", "dlinear_buffer_risk"]:
    ...
    metrics_df.to_csv(os.path.join(args.out_dir, "strategy_metrics_all_runs_dlinear.csv"), index=False, encoding="utf-8-sig")
    ...
    summary.to_csv(os.path.join(args.out_dir, "strategy_metrics_summary_dlinear.csv"), index=False, encoding="utf-8-sig")

    plot_nav(nav_dict, args.out_dir)

    print("=" * 80)
    print("DLinear Strategy summary")
    print("=" * 80)
    print(summary)
    print("=" * 80)
